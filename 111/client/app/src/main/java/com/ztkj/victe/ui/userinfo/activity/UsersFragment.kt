...
class UsersFragment : LazyFragment() {
    ...
    fun getData() {
        ...
        var method="getAllUsersByuserno"
        if(MyApplication.user.userLevel==1){
            map.put("userid","${MyApplication.user.userno}")
            method="getAllUsersByUserId"
        }else{
            map.put("userno","${MyApplication.user.userid}")
            method="getAllUsersByuserno"
        }
